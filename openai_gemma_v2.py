...
def reference_match_score(wanted: str, actual: str) -> int:
    if actual == wanted:
        return 4
    if actual.startswith(wanted) or wanted in actual:
        return 3
    if wanted.endswith(actual) and len(actual) >= 4:
        return 2
    if actual.endswith(wanted) and len(wanted) >= 4:
        return 1
    return 0


def representative_span_line(lines: list[Line]) -> Line:
    meaningful = [line for line in lines if len(norm(line.text)) >= 4]
    candidates = meaningful or lines
    return max(candidates, key=lambda line: (line.s, len(norm(line.text)), -line.order))
...
def match_reference_title(title: str, page_lines: list[Line]) -> Line | None:
    ...
    for line in page_lines:
        ...
        score = reference_match_score(wanted, actual)
        if not score:
            continue
        ranked.append((score, len(actual), line))
    if ranked:
        return max(ranked, key=lambda item: (item[0], item[1], item[2].s))[2]

    ordered = sorted(page_lines, key=lambda line: line.order)
    span_ranked: list[tuple[int, int, int, float, Line]] = []
    for start in range(len(ordered)):
        for size in range(2, min(4, len(ordered) - start) + 1):
            span = ordered[start:start + size]
            actual = norm(" ".join(line.text for line in span))
            score = reference_match_score(wanted, actual)
            if not score:
                continue
            representative = representative_span_line(span)
            span_ranked.append((
                score,
    ...
    return max(span_ranked, key=lambda item: item[:4])[4] if span_ranked else None
